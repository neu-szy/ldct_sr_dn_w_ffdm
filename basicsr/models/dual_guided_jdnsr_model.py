...
class DualGuidedModel(BaseModel):
    def __init__(self, opt):
        super(DualGuidedModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output_DNs, self.output_SRs = self.net_g(x=self.lq, mask=self.mask, avg_ct=self.avg_ct)
        ...
        if self.cri_pix_dn:
            ...
            for dn in self.output_DNs:
                l_pix_dn += self.cri_pix_dn(dn, self.gt_lr)
            l_total += l_pix_dn
            loss_dict['l_pix_dn'] = l_pix_dn
        if self.cri_pix_sr:
            l_pix_sr = 0
        ...
        if self.cri_perceptual:
            ...
            for sr in self.output_SRs:
                ...
                if l_percep_style[0] is not None:
                    l_percep += l_percep_style[0]
                    l_total += l_percep
                    loss_dict['l_percep'] = l_percep
                if l_percep_style[1] is not None:
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                _, self.output_SRs = self.net_g_ema(x=self.lq, mask=self.mask, avg_ct=self.avg_ct)
        else:
            self.net_g.eval()
            with torch.no_grad():
                _, self.output_SRs = self.net_g(x=self.lq, mask=self.mask, avg_ct=self.avg_ct)
            self.net_g.train()
...
@MODEL_REGISTRY.register()
class DualGuidedV2GANModel(BaseModel):
    def __init__(self, opt):
        super(DualGuidedV2GANModel, self).__init__(opt)
    ...
    def init_training_settings(self):
        ...
        if self.cri_pix_sr is None and self.cri_pix_dn is None and self.cri_tv is None and self.cri_perceptual is None and self.cri_gan is None:
            raise ValueError('Both pixel and perceptual losses are None.')
    ...
    def optimize_parameters(self, current_iter):
        ...
        l_d_real.backward()
        # fake
        fake_d_pred = self.net_d(self.output_SRs[-1].detach())
